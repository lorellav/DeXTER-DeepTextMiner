...
#   })
  ...
  observeEvent(select_data_network(), {
    ...
    updateSelectizeInput(session, "network_selected_node",
                         choices = unique(V(network)$name),
                         selected = ifelse(current.node %in% unique(V(network)$name), 
                                           current.node, 
                                           unique(V(network)$name)[1])
    )
  })
