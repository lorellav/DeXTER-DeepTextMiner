...
## 7. Network analysis
Network analysis is a great tool to study the patterns of collaboration between entities in the corpus, analyzing how entities interact with each other, rather than focussin on the entities themselves. For this project, we decided to analyze the co-occurrence of entities in the text. We built a co-occurrence network, where nodes represent entities cited in the corpus, which are connected if they appear in the same sentence. Therefore, edges represent the sentences in which entities co-occur. Several atributes are assigned to each edge/sentence, such as the issue containing the sentence, timestamp, co-occurrence frequency, sentence score and sentence magnitude. Likewise, each node/entity has several attributes, that are computed by aggregating all edges connected to it. For example, the occurrence freqency of an entity in the corpus is given by the sum of the co-occurrence frequencies of all dges connected to it, while sentiment score and magnitude are given by the mean value.
These interactions are not static, instead they change over time. The network is built in a way such that a filter on the timestamp can easily be appliead, obtaining a network that represents the interaction between entities in a specific time frame.
All the visualizations in the shiny app are based on this co-occurrence network. More specifically the shiny app provides two different visualizations. 
The first one, called ego network, focusses on entities. The ego network of an entity consist of the neighboring nodes of the entity and all edges connecting them. This is particurarly useful when the user is interested in a specific entity and how it interacts with other entities. The second one, called issue network, focusses on issues. In this case, the network consists of all nodes/entities that appear in one or more issues, chosen by the user, and all edge connecting them.
