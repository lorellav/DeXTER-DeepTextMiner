...
Even though sentiment can be considered a range, it is useful in some cases to define a clear distinction between positive and negative sentiment, for example in some of the visualizations in the dexter app. In this case, we considered sentiment to be positive for values higher than 0.3 and negative for values lower than -0.3, while everything in between was considered as neutral/mixed.  
In our analysis, we looked at the evolution of sentiment attached to entities and their relationships. We considered positive, negative and neutral sentiment. We did not use the information regarding magnitude, since differentiating between low sentiment and mixed sentiment was not in our interest.
