...
## 7. Network analysis
Network analysis is a great tool to study the patterns of collaboration between entities in the corpus, analyzing how entities interact with each other, rather than focussin on the entities themselves. For this project, we decided to analyze the co-occurrence of entities in the text. We built a co-occurrence network, where nodes represent entities cited in the corpus, which are connected if they appear in the same sentence. Therefore, edges represent the sentences in which entities co-occur. Several atributes are assigned to each edge/sentence, such as the issue containing the sentence, timestamp, co-occurrence frequency, sentiment score and magnitude. Likewise, each node/entity has several attributes, that are computed by aggregating all edges connected to it. For example, the occurrence freqency of an entity in the corpus is given by the sum of the co-occurrence frequencies of all edges connected to it, while sentiment score and magnitude are given by the mean value.
